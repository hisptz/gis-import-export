...
export class ResultsComponent implements OnInit {
	...
	//Returns uid of the selected Organisation Unit
	readMatch(event) {
		...
		let selectedPossibleArea = possibleList.parentElement.nextSibling.getElementsByTagName('strong')[0];
		let selectedPossibleOrgUnit = this.findOrgUnitFromFile(selectedChekBotton.id);
		...
		if (selectedChekBotton.checked == false) {
      selectedPossibleArea.innerHTML = '';
      selectedPossibleArea.parentElement.parentElement.setAttribute('hidden','')
		} else {
      selectedPossibleArea.innerHTML = selectedPossibleOrgUnit.name;
      selectedPossibleArea.parentElement.parentElement.removeAttribute('hidden')
		}

		//Testing
		//console.log(this.findOrgUnitFromFile(selectedChekBotton.id));

		return selectedChekBotton.id;
	}

	//Finds Organisation Unit from file given its uid
	findOrgUnitFromFile(uid: string) {
		for (let count = 0; count < this.matchResults.length; count++) {
			for (let duplicateCount = 0; duplicateCount < this.matchResults[count].results.length; duplicateCount++) {
				if (this.matchResults[count].results[duplicateCount].item.id == uid) {
					return this.matchResults[count].results[duplicateCount].item;
				}
			}
		}
	}
